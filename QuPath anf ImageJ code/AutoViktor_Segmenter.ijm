...
for (i = 0; i < subFolderList.length; i++) {
        ...
        for (j = 0; j < fileList.length; j++) {
                if (endsWith(fileList[j], ".jpeg")){
                        ...
                        run("Subtract Background...", "rolling=500 light"); //Removes the blurry dots on the image background
                        setMinAndMax(25, 255); //Adjusts the contrast of the image (makes the background lighter)
                        run("Apply LUT");
                        ...
                        run("Gaussian Blur...", "sigma=5"); //8 Makes the image slightly blurry
                        ...
                        run("Analyze Particles...", "size=5000-Infinity circularity=0.00-1.00 exclude include add"); //Performs a Particles Analysis on the image
                        selectWindow("processed");
                        close();
